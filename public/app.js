    ...
    if (parts[1]) {
        parts[1].split('\n').forEach(line => {
            ...
            if (key && val.length > 0) {
                ...
                // Extract the AI array separately from the standard metadata
...
const assembleVimeoDescription = (summary, metadata, aiGenerated) => {
    ...
    // Append the AI tag line ONLY if there are still items left in the array
...
const renderTable = (videos) => {
    ...
    videos.forEach(video => {
        ...
        // Save the aiGenerated array in memory so we can compare against it later
...
const handleSave = async (row) => {
    ...
    // Create a copy of the AI array so we can safely remove items from it
    let currentAiGenerated = [...(originalData.aiGenerated || [])];

    // If the human edited the summary, remove 'Description' from the AI list
    ...
    GOVERNANCE_KEYS.forEach(k => {
        ...
        if (cell) {
            ...
            // If the human edited a specific tag, remove that tag from the AI list
    ...
    // Rebuild the final text, which automatically hides the AI string if it's empty
    ...
    try {
        ...
        if (res.ok) { 
            ...
            // Update the local cache so hitting save twice doesn't cause bugs
