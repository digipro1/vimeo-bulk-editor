...
let currentVideosList = []; // New: maintains raw data for sorting
...
const parseVimeoDescription = (fullText) => {
    if (!fullText) return { summary: '', metadata: {}, aiGenerated: [] };
    ...
    if (parts[1]) {
        parts[1].split('\n').forEach(line => {
            ...
            if (key && val.length > 0) {
                ...
                if (trimmedKey === 'Generated by AI') {
                    aiGenerated = val.join(':').split(',').map(s => s.trim()).filter(Boolean);
                } else {
    ...
    return { summary, metadata, aiGenerated };
};

const assembleVimeoDescription = (summary, metadata, aiGenerated) => {
    let newDesc = summary + '\n---\n';
    GOVERNANCE_KEYS.forEach(key => {
        if (metadata[key]) newDesc += `${key}: ${metadata[key]}\n`;
    });
    if (aiGenerated && aiGenerated.length > 0) {
        newDesc += `Generated by AI: ${aiGenerated.join(', ')}\n`;
    }
    return newDesc.trim();
};
...
const handleSort = (key) => {
    ...
    currentVideosList.sort((a, b) => {
        let valA = '', valB = '';

        if (key === 'NativeTitle') {
            valA = a.name || ''; valB = b.name || '';
        } else if (key === 'Summary') {
            valA = parseVimeoDescription(a.description).summary;
            valB = parseVimeoDescription(b.description).summary;
        } else {
            valA = parseVimeoDescription(a.description).metadata[key] || '';
            valB = parseVimeoDescription(b.description).metadata[key] || '';
        }
...
const applyTableFilters = () => {
    ...
    let hasFilters = term !== '' || Object.keys(activeFilters).length > 0;
...
const fetchVideosByFolder = async () => {
    ...
    try {
        ...
        currentVideosList = Array.from(uniqueVideosMap.values());
        
        if (currentVideosList.length > 0) {
            // Reset sorting state
            sortState = { key: null, dir: 'asc' };
            document.querySelectorAll('.sort-label').forEach(el => el.classList.remove('sort-asc', 'sort-desc'));
            renderTable(currentVideosList);
        } else {
...
const renderTable = (videos) => {
    ...
    videos.forEach(video => {
        ...
        const { summary, metadata, aiGenerated } = parseVimeoDescription(video.description || '');
        
        originalVideoData.set(videoId, { summary, metadata, aiGenerated, name: video.name });
        ...
        // Ensure privacy is captured safely
        const currentPrivacy = video.privacy?.view || 'anybody';
            ...
            <td class="col-Media">
                <div class="media-card">
                    ...
                    <div class="media-actions">
                        <select class="privacy-select" title="Update Privacy">
                            ${privacyOptions.map(opt => `<option value="${opt}" ${currentPrivacy === opt ? 'selected' : ''}>${opt.charAt(0).toUpperCase() + opt.slice(1)}</option>`).join('')}
                        </select>
...
const handleSave = async (row) => {
    const videoId = row.dataset.videoId;
    const originalData = originalVideoData.get(videoId);
    const saveBtn = row.querySelector('.save-btn');
    saveBtn.innerText = 'Saving...';
    
    const summary = row.querySelector('.col-Summary').innerText.trim();
    const newPrivacy = row.querySelector('.privacy-select').value;
    const metadata = {};
    
    let currentAiGenerated = [...(originalData.aiGenerated || [])];

    if (summary !== originalData.summary) {
        currentAiGenerated = currentAiGenerated.filter(item => item !== 'Description');
    }

    GOVERNANCE_KEYS.forEach(k => {
        const cell = row.querySelector(`.meta-${k.replace(/\s+/g, '')}`);
        if (cell) {
            metadata[k] = cell.innerText.trim();
            if (metadata[k] !== (originalData.metadata[k] || '')) {
                currentAiGenerated = currentAiGenerated.filter(item => item !== k);
            }
        }
    });

    const finalDescription = assembleVimeoDescription(summary, metadata, currentAiGenerated);
    
    // Package updates payload: description + privacy
    const updates = { 
        description: finalDescription,
        privacy: { view: newPrivacy } 
    };
    ...
    try {
        ...
        if (res.ok) { 
            ...
            originalVideoData.set(videoId, { summary, metadata, aiGenerated: currentAiGenerated, name: originalData.name });
...
const handleBulkUpdate = () => {
    ...
    selectedVideoIds.forEach(id => {
        ...
        if (bulkPrivacy) {
            const privSelect = row.querySelector('.privacy-select');
            if (privSelect) privSelect.value = bulkPrivacy;
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.sort-label').forEach(label => {
        label.addEventListener('click', (e) => handleSort(e.target.dataset.sort));
    });
